...
int main(int argc,char **argv) {
  ...
  if (myrank == 0)
  {
    ...
    do
    {
      ...
      #pragma omp parallel for reduction(+:my_result_working) // reduction allow to execute operation on chosen variable in all threads
      for (long i=0;i<PACKET_SIZE;i++){
        long work_number = (*current_buffor)[i];
        my_result_working += is_prime(work_number);
      }
